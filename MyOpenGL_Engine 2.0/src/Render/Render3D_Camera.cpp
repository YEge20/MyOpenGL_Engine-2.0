#include"Render3D_Camera.h"

Render3D_Camera::Render3D_Camera(GLFWwindow* window)
	:m_window(window),m_rotadegree(0),
	m_camera_Pos(0.0f, 0.0f, 30.0f), m_camera_Fro(0.0f,0.0f,-1.0f),m_camera_Up(0.0f,1.0f,0.0f)
{
	std::cout << "Runnig Render3D_Camera.cpp" << std::endl;
	m_block1.reset(new DrawBlock(4.0f, 4.0f, 4.0f));
	m_block.reset(new DrawBlock(5.0f, 5.0f, 5.0f));
	m_block_shader.reset(new Shader(false, "./shader/Renderer3D_Camera/BasicShader(texture_version).shader"));

	m_testColorBlock.reset(new DrawBlock(5.0f, 5.0f, 5.0f));
	m_pureColorShader.reset(new Shader(false, "./shader/Renderer3D_Camera/BasicShader(pure_color).shader"));

	std::vector<std::string> face =
	{
		"./texture/crafttable/crafting_table_top.png",
		"./texture/crafttable/oak_planks.png",
		"./texture/crafttable/crafting_table_side.png",
		"./texture/crafttable/crafting_table_side.png",
		"./texture/crafttable/crafting_table_front.png",
		"./texture/crafttable/crafting_table_front.png"
	};
	std::vector<std::string> face1 =
	{
		"./texture/furnace/furnace_top.png",
		"./texture/furnace/furnace_top.png",
		"./texture/furnace/furnace_side.png",
		"./texture/furnace/furnace_side.png",
		"./texture/furnace/furnace_front_on.png",
		"./texture/furnace/furnace_side.png"
	};
	m_block1->ImportTexture(face, false, SlotID_1);
	m_block->ImportTexture(face1, false, SlotID_1);

	//3D渲染必须启动深度测试否侧渲染过程不正确：
	glEnable(GL_DEPTH_TEST);

	m_control_Camera = std::make_unique<KeyControlFor3D>(m_window,true,true);
}

void Render3D_Camera::renderContext(float timestep, float milltimestep)
{
	m_fov = m_control_Camera->MousescrollFunction();
	m_project = glm::perspective(glm::radians(45.0f + m_fov), (float)1200 / (float)800, 0.1f, 450.0f);
	glm::mat4 CameraView = m_control_Camera->CameraMove(&m_camera_Pos, &m_camera_Fro, &m_camera_Up, 12.5f * timestep);

	glViewport(0, 0, 1200, 800);
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	glClearColor(0.1f, 0.1f, 0.1f, 0.5f);
	
	CraftTable(m_project, CameraView, m_camera_Pos);
	Furance(m_project, CameraView, m_camera_Pos);
	ColorBlock(m_project, CameraView, m_camera_Pos);
}

void Render3D_Camera::renderImguiContext()
{
	ImGui::SliderFloat(u8"每个对象的旋转角度", &m_rotadegree, -90.0f, 90.0f,"%.2f");
	ImGui::Text("camera position: x:%.2f  y:%.2f  z:%2.f", m_camera_Pos.x, m_camera_Pos.y, m_camera_Pos.z);
	ImGui::Text("fov: %.2f", m_fov + 45.0f);
}

void Render3D_Camera::CraftTable(glm::mat4& project, glm::mat4& Camera, glm::vec3& Cam_Pos)
{
	m_block_shader->bind();
	m_block_shader->setuniform1i("u_texture", 0);
	glm::mat4 rotation = glm::rotate(glm::mat4(1.0f), glm::radians(m_rotadegree), glm::vec3(-1.0f, 1.0f, 0.0f));
	glm::mat4 position = glm::translate(glm::mat4(1.0f), moreObject_CraftTable);
	glm::mat4 model = position * rotation;
	glm::mat4 MVP1 = project * Camera * model;
	m_block_shader->setuniformMat4f("u_MVP", MVP1);
	m_block1->DrawCallTexture(0, 0, 0);
}

void Render3D_Camera::ColorBlock(glm::mat4 &project, glm::mat4 &Camera, glm::vec3 &Cam_Pos)
{
	m_pureColorShader->bind();
	glm::mat4 rotation = glm::rotate(glm::mat4(1.0f), glm::radians(m_rotadegree), glm::vec3(0.0f, 1.0f, 1.0f));
	glm::mat4 position = glm::translate(glm::mat4(1.0f), m_colorBlock_Pos);
	glm::mat4 model = position * rotation;
	glm::mat4 MVP = project * Camera * model;
	m_pureColorShader->setuniformMat4f("u_MVP", MVP);
	m_testColorBlock->DrawCall();
}

void Render3D_Camera::Furance(glm::mat4& project, glm::mat4& Camera, glm::vec3& Cam_Pos)
{
	m_block_shader->bind();
	m_block_shader->setuniform1i("u_texture", 0);
	glm::mat4 rotation = glm::rotate(glm::mat4(1.0f), glm::radians(m_rotadegree), glm::vec3(1.0f, 1.0f, 0.0f));
	glm::mat4 position = glm::translate(glm::mat4(1.0f), moreObject);
	glm::mat4 model = position * rotation;
	glm::mat4 MVP = project * Camera * model;
	m_block_shader->setuniformMat4f("u_MVP", MVP);
	m_block->DrawCallTexture(0, 0, 0);
}
