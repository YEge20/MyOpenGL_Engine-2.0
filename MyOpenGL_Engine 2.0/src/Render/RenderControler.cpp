...
int RenderControler::renderContextControl(float timestep, float milltimestep)
{
	...
	if (m_singleObject == 0)
		...
		m_ImportModel->renderContext(timestep, milltimestep);
	else
		return -1;
}
...
void RenderControler::renderImguiContextControl()
{
	...
	if (m_singleObject == 0)
		...
		m_ImportModel->renderImguiContext();
}
...
int RenderControler::AutoRenderControl(int projectnum)
{
	...
	if (m_singleObject == 0)
		...
		m_ImportModel = std::make_unique<ImportModel>(m_window);
	else
	{
		system("color 04");
...
int RenderControler::MyselfRenderControl()
{
	...
	std::cout << "#5                 ShadowMap2.cpp\n";
	std::cout << "#6                 stencil_test.cpp\n";
	std::cout << "#7                 DynamicTextureBuffer.cpp\n";
	std::cout << "#8                 Pointlight_ShadowCubemap.cpp\n";
	std::cout << "#9                 ImportModel.cpp\n";
