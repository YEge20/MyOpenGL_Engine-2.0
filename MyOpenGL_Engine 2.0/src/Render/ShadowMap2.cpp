...

{
	...
	m_shader.reset(new Shader(false, "./shader/depth2/BasicShader3D(Light_version)3.shader"));
	m_shader_1.reset(new Shader(false, "./shader/depth2/GrassGround.shader"));
	m_shader_2.reset(new Shader(false, "./shader/depth2/Grass.shader"));
	m_shader_depth.reset(new Shader(false, "./shader/depth2/depthMap.shader"));

	m_rain_shader.reset(new Shader(false, "./shader/depth2/Rain.shader"));
	m_rain_drop.reset(new DrawBlock(1.0f,3.0f,1.0f));

	m_rainPartical_shader.reset(new Shader(false, "./shader/depth2/RainPartical.shader"));
	m_rain_partical.reset(new DrawBlock(1.5f, 1.5f, 1.5f));

	m_reflect_shader.reset(new Shader(false, "./shader/depth2/WaterReflect.shader"));
...
//平行光的位置、亮度的计算
	...
	if (m_degreeOflight_speed_current < m_degreeOflight_speed)
		m_degreeOflight_speed_current += 2.5f * timestep;
	else if (m_degreeOflight_speed_current > m_degreeOflight_speed)
		m_degreeOflight_speed_current -= 2.5f * timestep;
	m_degreeOflight += m_degreeOflight_speed_current * timestep;
...
//深度贴图计算
	...
	if (m_speed_add < m_rotaspeed)
		m_speed_add += 2.5f * timestep;
	else if (m_speed_add > m_rotaspeed)
		m_speed_add -= 2.5f * timestep;
	m_rad += m_speed_add * timestep;
	if (m_rad > 360.0f)
		m_rad = 0.0f;
	//矿石的漂浮计算
	m_float_time += 120.0f * timestep;
	...
	if (m_GrassFlower_wave_current > m_GrassFlower_wave)
		m_GrassFlower_wave_current -= 0.1f * timestep;
	else if (m_GrassFlower_wave_current < m_GrassFlower_wave)
		m_GrassFlower_wave_current += 0.1f * timestep;

	//花草的摇摆频率计算
	if (m_temperature > 0.6f)
		m_GrassFlower_Frequency = 90.0f;
	else if (m_temperature > -0.75f && m_temperature < -0.5f)
		m_GrassFlower_Frequency = 150.0f;
	else if (m_temperature < -0.75f)
		m_GrassFlower_Frequency = 180.0f;
	else
		m_GrassFlower_Frequency = 120.0f;
	if (m_GrassFlower_Frequency_current > m_GrassFlower_Frequency)
		m_GrassFlower_Frequency_current -= 120.0f * timestep;
	else if (m_GrassFlower_Frequency_current < m_GrassFlower_Frequency)
		m_GrassFlower_Frequency_current += 120.0f *timestep;
	if (m_GrassFlower_shake_degree > 360.0f)
		m_GrassFlower_shake_degree = 0.0f;
	m_GrassFlower_shake_degree += m_GrassFlower_Frequency_current * timestep;
...
//渲染缓冲帧：
	...
	glm::mat4 ortho_project = m_reflect_texture->Caculate_Ortho_Project(160.0f, 160.0f, 500.0f);
...
//渲染物体
	...
	for (int y = 0;y < m_drop_lay_count; y++)
	{
		...
		if (m_rain_drop_controler == true)
			m_drop_Yposition[y] -= m_drop_speed * timestep;
	}
	//开始下雨过度：
	...
	{
		m_partical_alpha += 1.0f * timestep;
		if (m_partical_alpha > 1.0f)
			m_partical_alpha = 1.0f;
		if (m_rain_drop_controler == true)
		{
			if (m_partical_tick < 360.0f)
				m_partical_tick += timestep * 580.0f;//开始下雨后就开始计时
			if (m_partical_tick >= 360.0f)
				m_partical_tick = 0.0f;
			for (int x = 0; x < 4; x++)//计算坐标值
			{
				m_partical[x] += timestep * 10.0f * m_partical_front[x];
	...
	if (m_rain_occure_controler == false)//下雨结束时执行的数据更新方案
	{
		...
		if (m_partical_tick < 360.0f)
			m_partical_tick += timestep * 580.0f;
		if (m_partical_tick >= 360.0f)
			m_partical_tick = 0.0f;
		for (int x = 0; x < 4; x++)//计算坐标值
		{
	...
	if (m_reflect_alpha < m_reflect_alpha_setup)
		m_reflect_alpha += 0.1f * timestep;
	if (m_reflect_alpha > m_reflect_alpha_setup)
		m_reflect_alpha -= 0.2f * timestep;
...
void ShadowMap2::renderImguiContext()
{
	...
	if (m_GUIbutton_time_speed_setup == true)
	{
		...
		ImGui::SliderFloat("time speed", &m_degreeOflight_speed, 0.0f, 10.0f);
		if (ImGui::Button("reset time speed"))
			m_degreeOflight_speed = 5.0f;
	}
	...
	if (m_GUIbutton_rain_event_setup == true)
	{
		...
		ImGui::SliderFloat("drop speed", &m_drop_speed, 100.0f, 200.0f);
	...
	if (ImGui::Button("reset all"))
	{
		...
		m_degreeOflight_speed = 5.0f;
		m_degreeOflight_speed_current = 5.0f;
		m_temperature = 0.0f;
		m_count_day = 0.0f;
