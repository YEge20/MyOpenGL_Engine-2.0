...
stencil_test::stencil_test(GLFWwindow* window)
	:m_window(window),
	...
	m_camera_Pos(0.0f, 41.0f, 25.0f), m_camera_Fro(0.0f, 0.0f, 1.0f), m_camera_Up(0.0f, 1.0f, 0.0f)
{
	...
	m_shader.reset(new Shader(false, "./shader/light/BasicShader3D_object.shader"));
	m_shader1.reset(new Shader(false, "./shader/light/BasicShader3D_light_object.shader"));
	m_shader_stencil.reset(new Shader(false, "./shader/light/StencilTest.shader"));
	m_shaderSkybox.reset(new Shader(false, "./shader/light/SkyboxShader.shader"));
	//VAO 0的设置：
...
//渲染光源并且执行模板测试(目前只能放到最后渲染才可以让X-ray功能与模板测试功能完全正常运行)：
	glStencilOp(GL_KEEP, GL_KEEP, GL_REPLACE);//模板位数的更新规则
	glStencilMask(0xff);//写入模板值,并应用该模板

	glStencilFunc(GL_ALWAYS, 1, 0xff);//确保每次模板数值写入都能被通过，并且让对应的模板位数等于1,设置该模板的掩码为0xff
	m_shader1->bind();
	m_texture32->unbind();
	...
	glStencilFunc(GL_NOTEQUAL, 1, 0xff);//可以防止模板绘制的图形将原来的图形给挡住
	if (m_stencil_Xray == true)
		glDisable(GL_DEPTH_TEST);//设置这个可以让模板测试的物体实现透视功能
	if (m_option_stencil == true)
	{
		m_shader_stencil->bind();
		m_shader_stencil->setuniformVEC3("u_stencil_color", sin(1.2 * glfwGetTime()), sin(1.2 * glfwGetTime() + glm::radians(90.0f)), sin(1.2 * glfwGetTime() + glm::radians(180.0f)));
		glm::mat4 scalProject1 = glm::scale(glm::mat4(1.0f), glm::vec3(1.2f, 1.2f, 1.2f));
		for (int i = 0; i < 6; i++)
		{
			glm::mat4 move1 = glm::translate(glm::mat4(1.0f), m_move1[i] * m_Plight_dis);
			glm::mat4 MVP2 = m_project * CameraView * move1 * scalProject1;
			m_shader_stencil->setuniformMat4f("u_MVP", MVP2);
			glDrawElements(GL_TRIANGLES, LayoutNum_1, GL_UNSIGNED_INT, nullptr);
		}
	}
	glStencilMask(0xff);//写入数值进入模板，应用该模板
	if (m_stencil_Xray == true)
		glEnable(GL_DEPTH_TEST);
	glStencilOp(GL_KEEP, GL_KEEP, GL_KEEP);//模板位数的更新规则恢复默认状态
}

void stencil_test::renderImguiContext()
{
	ImGui::SliderFloat3("X Y Z", &m_move.x, -300.0f, 300.0f);
	ImGui::SliderFloat("skybox light", &m_skyboxLight, 0.0f, 0.7f);
	ImGui::SliderFloat("Block distance", &m_object_dis, 1.0f, 1.5f);
	if (ImGui::Button("reset position"))
		m_move = { 0.0f, 0.0f, 0.0f };
	if (m_option_stencil == false)
	{
		if (ImGui::Button("turn on pointlight stencil_test"))
			m_option_stencil = true;
	}
	else if (m_option_stencil == true)
	{
		if (ImGui::Button("turn off pointlight stencil_test"))
			m_option_stencil = false;
	}
	if (m_option_stencil == true)
	{
		ImGui::SliderFloat("PointLight distance", &m_Plight_dis, 1.0f, 2.5f);
		if (m_stencil_Xray == false)
		{
			if (ImGui::Button("Turn on block_light X-Ray"))
				m_stencil_Xray = true;
		}
		else
		{
			if (ImGui::Button("Turn off block_light X-Ray"))
				m_stencil_Xray = false;
		}
