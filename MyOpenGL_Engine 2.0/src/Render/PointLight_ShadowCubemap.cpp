	...
	:m_window(window)
{
	...
	m_sky_shader.reset(new Shader(false, "./shader/Pointlight_ShadowCubemap/Sky.shader"));
	...
	m_glass_shader.reset(new Shader(false, "./shader/Pointlight_ShadowCubemap/Reflect.shader"));
...
void Pointlight_ShadowCubemap::renderContext(float timestep, float milltimestep)
{
	...
	glViewport(0, 0, m_WindowsizeControl->GetWidth(), m_WindowsizeControl->GetHeight());

	m_fov = m_camera_controler->MousescrollFunction();
	m_projeect = glm::perspective(glm::radians(45.0f + m_fov), (float)m_WindowsizeControl->GetWidth() / (float)m_WindowsizeControl->GetHeight(), 0.1f, 800.0f);
