...
ShadowMap::ShadowMap(GLFWwindow* window)
	:m_window(window), m_rotaspeed(0.1f),
	m_move(0.0f, -10.0f, 0.0f),m_degreeOflight(0.0f),m_degreeOflight_speed(0.05f),m_degreeOflight_speed_current(0.0f), m_light_color{1.0f,1.0f,1.0f},
	...
	m_grass_move(0.0f, -120.0f, 0.0f),m_count_day(0), m_temperature(0.0f), m_grass_2_move(0.0f, -109.5f, 0.0f), m_flower_move(0.0f, -112.0f, 0.0f), m_france_position(-40.0f, 0.6f, 55.4f),
...
//平行光的位置、亮度的计算
	...
	if (m_degreeOflight > 90.0f && m_day == false)//夜晚
	{
		m_degreeOflight = -90.0f;
...
//深度贴图计算
	...
	for (int i = 0; i < 10; i++)
	{
		...
		glm::mat4 wave0 = glm::translate(glm::mat4(1.0f), glm::vec3((1.0f + m_GrassFlower_wave_current) * sin(glfwGetTime() + m_moreObject_2[i].x * m_moreObject_2[i].z), 0.0f, (1.0f + m_GrassFlower_wave_current) * cos(glfwGetTime() + m_moreObject_2[i].x * m_moreObject_2[i].z)));
	...
	for (int i = 0; i < 10; i++)
	{
		...
		glm::mat4 wave1 = glm::translate(glm::mat4(1.0f), glm::vec3((1.0f + m_GrassFlower_wave_current) * sin(glfwGetTime() + m_moreObject_3[i].x * m_moreObject_3[i].z), 0.0f, (1.0f + m_GrassFlower_wave_current) * cos(glfwGetTime() + m_moreObject_3[i].x * m_moreObject_3[i].z)));
...
void ShadowMap::renderImguiContext()
{
